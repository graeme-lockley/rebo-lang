...
pub fn open(machine: *Helper.ASTInterpreter, numberOfArgs: usize) !void {
    ...
    if (options.v == Helper.ValueKind.UnitKind) {
        try machine.runtime.push(try machine.runtime.newFileValue(std.fs.cwd().openFile(path, .{}) catch |err| return Helper.raiseOsError(machine, "open", err)));
        return;
    }

    const readF = try booleanOption(machine.runtime.stringPool, options, "read", false);
    const writeF = try booleanOption(machine.runtime.stringPool, options, "write", false);
    const appendF = try booleanOption(machine.runtime.stringPool, options, "append", false);
    const truncateF = try booleanOption(machine.runtime.stringPool, options, "truncate", false);
    const createF = try booleanOption(machine.runtime.stringPool, options, "create", false);
    ...
    if (createF) {
        ...
        try machine.runtime.push(try machine.runtime.newFileValue(file));
    } else {
